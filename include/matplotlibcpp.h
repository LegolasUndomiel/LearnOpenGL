...
// template <> struct select_npy_type<long long> { const static NPY_TYPES type =
// NPY_INT64; };
static_assert(sizeof(unsigned long long) == 8);
// template <> struct select_npy_type<unsigned long long> { const static
...
bool contour(const std::vector<NumericX> &x, const std::vector<NumericY> &y,
    ...
    PyObject *res =
        PyObject_Call(detail::_interpreter::get().s_python_function_contour,
                      plot_args, kwargs);
...
inline void tick_params(const std::map<std::string, std::string> &keywords,
    ...
    PyObject *res =
        PyObject_Call(detail::_interpreter::get().s_python_function_tick_params,
                      args, kwargs);
